...
/** The type definition of homomorphism */
...
case class Lang(
  ...
  def complement: Lang = Lang(
    this.symbols,
    w => !this.contains(w),
  )

  /** Intersection operation */
  def intersect(that: Lang): Lang = Lang(
    this.symbols intersect that.symbols,
    w => this.contains(w) && that.contains(w),
  )

  /** Reverse operation */
  def reverse: Lang = Lang(
    this.symbols,
    w => this.contains(w.reverse),
  )
